class MulungushiLiveClass extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === "live") {
      this.setLiveStatus(newValue === "true");
    }
  }
  ...
  render() {
    ...
    this.setLiveStatus(this.isLive);
  }
  ...
  // Called when the "live" attribute changes (set from Wix page code)
  setLiveStatus(isLive) {
    ...
    if (!badge || !btn || !statusText) return; // not rendered yet
